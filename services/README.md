...
The `services/` package houses the external integrations that feed or actuate the agent. Planner code should prefer importing these modules directly instead of shelling out to external commands.

## 🌤️ Weather 

National Weather Service forecast integration is implemented in-repo under [weather](/Users/jason/Local/school/590-agents/project2/agent-bakeoff/services/weather/README.md).
...
Orbit B-hyve irrigation control is implemented in-repo under [bhyve](/Users/jason/Local/school/590-agents/project2/agent-bakeoff/services/bhyve/README.md). This service provides the actuation path used by the agent for lawn watering.
...
The RTSP helper now lives in [rtsp](/Users/jason/Local/school/590-agents/project2/agent-bakeoff/services/rtsp/README.md). Its main entry point is [rtsp/ingest.py](/Users/jason/Local/school/590-agents/project2/agent-bakeoff/services/rtsp/ingest.py). It uses `ffmpeg` to decode a live stream into raw `bgr24` frames and hands those frames to an OpenCV loop. The script is intended to be the bridge from shell-level stream validation to agent perception logic.
